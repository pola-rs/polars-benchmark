...
def q():
    ...
    def query():
        ...
        lineitem_filtered = line_item_ds[
            (line_item_ds["l_shipdate"] >= datetime.strptime("1995-01-01", '%Y-%m-%d').date()) &
            (line_item_ds["l_shipdate"] < datetime.strptime("1997-01-01", '%Y-%m-%d').date())
            ]
        ...
        lineitem_filtered = lineitem_filtered.loc[:, ["l_orderkey", "l_suppkey", "l_year", "revenue"]]
        ...
        n1 = nation_ds[(nation_ds["n_name"] == "FRANCE")].loc[:, ["n_nationkey", "n_name"]]
        n2 = nation_ds[(nation_ds["n_name"] == "GERMANY")].loc[:, ["n_nationkey", "n_name"]]
        ...
        result_df = total.groupby(["supp_nation", "cust_nation", "l_year"]).revenue.agg("sum").reset_index()
