...
def q():
    ...
    with CodeTimer(name=f"Get result of Query {Q_NUM}", unit='s'):
        ...
        partsupp_filtered = part_supp_ds.loc[:, ["ps_partkey", "ps_suppkey", "ps_supplycost"]]
        ...
        ps_s_r_n_merged = ps_s_r_n_merged.loc[:,
                          ["n_name", "s_name", "s_address", "s_phone", "s_acctbal", "s_comment", "ps_partkey",
                           "ps_supplycost"]]
        part_filtered = part_ds.loc[:, ["p_partkey", "p_mfgr", "p_size", "p_type"]]
        part_filtered = part_filtered[(part_filtered["p_size"] == var1) & (part_filtered["p_type"].str.endswith(var2))]
        ...
        merged_df = merged_df.loc[:,
                    ["n_name", "s_name", "s_address", "s_phone", "s_acctbal", "s_comment", "ps_supplycost", "p_partkey",
                     "p_mfgr"]]
        min_values = merged_df.groupby("p_partkey", as_index=False)["ps_supplycost"].min()
        min_values.columns = ["P_PARTKEY_CPY", "MIN_SUPPLYCOST"]
        merged_df = merged_df.merge(min_values, left_on=["p_partkey", "ps_supplycost"],
                                    right_on=["P_PARTKEY_CPY", "MIN_SUPPLYCOST"], how="inner")
        result_df = merged_df.loc[:,
                    ["s_acctbal", "s_name", "n_name", "p_partkey", "p_mfgr", "s_address", "s_phone", "s_comment"]]
        result_df = result_df.sort_values(by=["s_acctbal", "n_name", "s_name", "p_partkey", ],
                                          ascending=[False, True, True, True, ])[:100]
