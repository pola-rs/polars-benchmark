...
def q():
    ...
    def query():
        ...
        lineitem_filtered = line_item_ds[
            (line_item_ds["l_shipdate"] >= np.datetime64("1995-01-01"))
            & (line_item_ds["l_shipdate"] < np.datetime64("1997-01-01"))
        ]
        ...
        orders_filtered = orders_ds[["o_orderkey", "o_custkey"]]
        customer_filtered = customer_ds[["c_custkey", "c_nationkey"]]
        n1 = nation_ds[(nation_ds["n_name"] == "FRANCE")][["n_nationkey", "n_name"]]
        n2 = nation_ds[(nation_ds["n_name"] == "GERMANY")][["n_nationkey", "n_name"]]

        # ----- do nation 1 -----
        N1_C = customer_filtered.join(
            n1,
            left_on="c_nationkey",
        ...
        N1_C = N1_C.drop(columns=["c_nationkey", "n_nationkey"])
        N1_C.rename("n_name", "cust_nation")
        ...
        N2_S = N2_S.drop(columns=["s_nationkey", "n_nationkey"])
        N2_S.rename("n_name", "supp_nation")
        N2_S_L = N2_S.join(
            lineitem_filtered,
            left_on="s_suppkey",
            right_on="l_suppkey",
        ...
        N2_C = N2_C.drop(columns=["c_nationkey", "n_nationkey"])
        N2_C.rename("n_name", "cust_nation")
        N2_C_O = N2_C.join(
            orders_filtered,
            left_on="c_custkey",
            right_on="o_custkey",
        ...
        N1_S = N1_S.drop(columns=["s_nationkey", "n_nationkey"])
        N1_S.rename("n_name", "supp_nation")
        N1_S_L = N1_S.join(
            lineitem_filtered,
            left_on="s_suppkey",
            right_on="l_suppkey",
        ...
        # concat results
        total = vaex.concat([total1, total2])
        result_df = total.groupby(["supp_nation", "cust_nation", "l_year"]).agg(
            {"revenue": "sum"}
        )
